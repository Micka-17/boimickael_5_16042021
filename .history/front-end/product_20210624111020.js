...
//////////////// ajout de produit
    ...
    btnBasket.addEventListener("click", (event)=>{
        ...
        let productArray = [infoProduit];
        console.log("les infos " + infoProduit);
        //// Save allValue of selected option 
        if (localStorage.length === 0) {
            localStorage.setItem("product", JSON.stringify(productsInCartArray));
            ...
            let productsInCartArray = JSON.parse(localStorage.getItem("product"));
            //Ajout panier
            if (JSON.stringify(productsInCartArray).indexOf(JSON.stringify(infoProduit)) === -1) {
                productsInCartArray.push(infoProduit);
