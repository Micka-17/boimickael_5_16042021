  ...
  let empty = document.createElement("p");
    ...
    for (let i = 0; i < savedValue.length; i++) {
      ...
      let garbadge = document.createElement("div");
        ...
        min="1" max="3">
        <button class="btn plus-btn${savedValue[i].numberId} " type="button">+</button><button class="btn deleteMe"><i class="far fa-trash-alt"></i></button>`; //savedValue[i].numberId + 
      ...
      // for (let j = 0; j < optionPresent.length; j++) {
       ...
       document.querySelector(`".plus-btn${savedValue[i].numberId}"`).addEventListener("click", function() {
           ...
           if(valueCount >= 3)
           {
               document.querySelector(`".plus-btn${savedValue[i].numberId}"`).setAttribute("disabled", "disabled");
           } 
       ...
       document.querySelector(".minus-btn").addEventListener("click", function() {
           ...
           if(valueCount < 3)
           {
               document.querySelector(`".plus-btn ${savedValue[i].numberId}"`).removeAttribute("disabled");
               document.querySelector(`".plus-btn ${savedValue[i].numberId}"`).classList.remove("disabled");
