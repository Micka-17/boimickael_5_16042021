...
fetch('http://localhost:3000/api/cameras')
  ...
  .then((cameras) => {
    ...
    for(let i = 0; i < cameras.length; i++) {
      ...
      html += `<div class="d-flex","justify-content-evenly", "flex-wrap">
      <h2 class="row">${cameras[i].name}</h2>
      <p class="row"><img src="${cameras[i].imageUrl}" alt="Images ours" style= "width:28rem; border-radius:5px;"></p>
      <p class="row">${cameras[i].description}</p>
      <p class="row">${(cameras[i].price/100).toFixed(2).replace(".",",")}€</p>
      <a class="row" href="./produit.html?${cameras[i]._id}" style= "background-color:#f3e9f1; box-shadow: 0px 0px 12px 0px white; margin:auto; width:100px; border-radius:15px; padding:10px;"><b>Voir l'article</b></a></li>`
    }
