  ...
  let empty = document.createElement("p");
    ...
    for (let i = 0; i < savedValue.length; i++) {
      ...
      let garbadge = document.createElement("div");
        ...
        <input type="text" id="quantity ${savedValue[i].numberId}" value="${savedValue[i].productQuantity}" placeholder="Maximum 3" 
        min="1" max="3">
      ...
      // for (let j = 0; j < optionPresent.length; j++) {
       ...
       document.querySelector(".plus-btn").addEventListener("click", function() {
           valueCount = document.getElementById(`"quantity ${savedValue[i].numberId}"`).value;
           valueCount++;
           console.log("0" + valueCount);
           document.getElementById("quantity ${savedValue[i].numberId}").value = valueCount;
