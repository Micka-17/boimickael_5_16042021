...
//// faire une boucle for
    ...
    lens1.textContent = selectedProduct.lenses[1];

    const lens2 = document.createElement("option");
    addlens.appendChild(lens2);
    lens2.textContent = selectedProduct.lenses[2];

    const addToBasket = document.createElement("p");
    priceselected.appendChild(addToBasket);
    addToBasket.classList.add("d-flex","justify-content-evenly");
    addToBasket.innerHTML = `<a class="add" href="./basket.html?id=${cameras[i]._id}"><i class="fas fa-cart-arrow-down"></i></a>`;
}
