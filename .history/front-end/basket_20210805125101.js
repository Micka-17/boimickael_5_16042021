...
// Le panier est non vide alors mise en pages du panier avec les produits (boucle for)
  ...
  console.log(totalPrices);
  localStorage.setItem("totalPrice", JSON.stringify(totalPrices));
};
...
btnForm.addEventListener("click", (e) => {
  ...
  if (
    ...
    (regexlastName.test(contactForm.lastName) == true) &
    (regexNum.test(contactForm.num) == true) &
    (regexEmail.test(contactForm.email) == true) &
    (regexaddress.test(contactForm.address) == true) &
    (regexCodePostale.test(contactForm.codePostale) == true) &
    (regexCity.test(contactForm.city) == true)
  ) {
    let data = {};
    ...
    let products = [];
    for (let m = 0; m < savedValue.length; m++){
      let productsId = savedValue[m].numberId;
    ...
    console.log(contactForm.address)
    ...
    fetch("http://localhost:3000/api/cameras/order", {
                ...
                .then((data) => {
                    localStorage.setItem("order", JSON.stringify(allData.orderId));
