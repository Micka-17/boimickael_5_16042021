  ...
  let empty = document.createElement("p");
    ...
    for (let i = 0; i < savedValue.length; i++) {
      ...
      // for (let j = 0; j < optionPresent.length; j++) {
       ...
       buttonplus = document.querySelectorAll(".plus-btn");
       for (i = 0; i < buttonsplus.length; i++) {
           element = buttonsplus[i];
       ...
       buttonsminus = document.querySelectorAll(".minus-btn");
       for (i = 0; i < buttonsminus.length; i++) {
           element = buttonsminus[i];
           element.addEventListener("click", function(e) {
          e.stopImmediatePropagation();
          e.preventDefault();
