      ...
      // for (let j = 0; j < optionPresent.length; j++) {
       ...
       for (i = 0; i < buttonsadd.length; i++) {
          ...
          e.preventDefault();
           ...
           if (valueCount > 1) {
            this.querySelectorAll(".minus-btn").removeAttribute("disabled");
            this.querySelectorAll(".minus-btn").classList.remove("disabled");
           } 
           if (valueCount >= 3) {
       ...
       buttonsremove = document.querySelectorAll(".plus-btn");
       for (i = 0; i < buttonsremove.length; i++) {
           element = buttonsremove[i];
           element.addEventListener("click", function(e) {
          e.stopImmediatePropagation();
          e.preventDefault();
           ...
           if(valueCount <= 1)
           {
            this.querySelectorAll(".minus-btn").setAttribute("disabled", "disabled");
           } 
           if(valueCount < 3)
           {
            this.querySelectorAll(".plus-btn").removeAttribute("disabled");
            this.querySelectorAll(".plus-btn").classList.remove("disabled");
           } 
});}
